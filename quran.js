  ...
  async function fetchSurahs() {
    try {
      ...
      data.chapters.forEach((surah) => {
        ...
        surahItem.className =
          'p-3 rounded-lg cursor-pointer hover:bg-green-50 transition duration-200';
        surahItem.innerHTML = `
      ...
      surahList.innerHTML =
        '<p class="text-red-500">Failed to load surahs. Please try again later.</p>';
    }
  }
  ...
  async function loadSurah(surahId) {
    try {
      ...
      const response = await fetch(
        `https://quranapi.pages.dev/api/chapters/${surahId}/verses`
      );
      const data = await response.json();
      ...
      surahInfo.innerHTML =
        '<p class="text-red-500">Failed to load surah. Please try again later.</p>';
    }
  }
  ...
  function displayVerses() {
    ...
    versesToShow.forEach((verse) => {
      ...
      verseElement.className =
        'mb-8 pb-8 border-b border-gray-100 last:border-0 last:pb-0 last:mb-0';
...
const QURAN_API_URL = 'https://api.alquran.cloud/v1/ayah/random';

// Hadith API (HadithAPI.com)
const HADITH_API_URL = 'https://hadithapi.com/api/hadiths/random';
const HADITH_API_KEY = 'YOUR_API_KEY_HERE'; // Replace with your actual key or use process.env.HADITH_API_KEY on Vercel
...
async function loadQuranAndHadith() {
  ...
  try {
    ...
    const quranRes = await fetch(QURAN_API_URL);
